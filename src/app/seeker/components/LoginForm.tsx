    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-y-4">
      <div className="group">
        ...
        <div className="relative">
          ...
          {errors.phoneNumber ? (
            <p className="text-red-600">{translate(errors.phoneNumber)}</p>
          ) : null}
        </div>
      </div>
      <div className="group">
        ...
        <div className="relative">
          <input
            ...
            placeholder={translate("Последний код полученный по СМС")}
            className="w-full px-4 py-4 pl-12 border-2 border-gray-200 rounded-xl focus:border-green-400 focus:ring-4 focus:ring-green-100 transition-all duration-200 outline-none bg-gray-50 focus:bg-white text-lg"
            required
          />
          <Lock className="absolute left-4 top-1/2 transform -translate-y-1/2 text-gray-400" size={20} />
          {errors.password ? (
            <p className="text-red-600">{translate(errors.password)}</p>
          ) : null}
        </div>
      </div>
