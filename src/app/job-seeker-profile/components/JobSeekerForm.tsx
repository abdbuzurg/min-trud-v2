    ...
    switch (step) {
      ...
      case 5:
        formData.workExperience.forEach((exp, index) => {
          ...
          if (!exp.responsibilities.trim()) newErrors[`responsibilities_${index}`] = 'Обязательное поле';
        });
        break;
      case 6:
        if (!formData.preferredCountry.trim()) newErrors.preferredCountry = 'Обязательное поле';
        if (!formData.preferredPosition.trim()) newErrors.preferredPosition = 'Обязательное поле';
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <textarea
                ...
                className={`w-full px-4 py-3 border-2 rounded-xl focus:ring-4 focus:ring-green-100 transition-all duration-200 outline-none resize-none ${errors[`responsibilities_${index}`] ? 'border-red-300 focus:border-red-400' : 'border-gray-200 focus:border-green-400'
                  }`}
                rows={3}
                placeholder="Опишите ваши обязанности"
              />
              {errors[`responsibilities_${index}`] && <p className="text-red-500 text-sm mt-1">{errors[`responsibilities_${index}`]}</p>}
            </div>
          </div>
        </div>
      ))}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          ...
          <input
            ...
            className={`w-full px-4 py-3 border-2 rounded-xl focus:ring-4 focus:ring-green-100 transition-all duration-200 outline-none ${errors.preferredPosition ? 'border-red-300 focus:border-red-400' : 'border-gray-200 focus:border-green-400'
              }`}
            placeholder="Введите желаемую должность"
          />
          {errors.preferredPosition && <p className="text-red-500 text-sm mt-1">{errors.preferredPosition}</p>}
        </div>
